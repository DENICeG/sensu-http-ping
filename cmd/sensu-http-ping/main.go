...
func main() {
	...
	if err != nil {
		fmt.Println("ERR: failed to prepare request:", err.Error())
		os.Exit(1)
	}

	resp, err := client.Do(req)
	if err != nil {
		fmt.Println("ERR: request failed:", err.Error())
		os.Exit(1)
	}

	if fail && (resp.StatusCode < 200 || resp.StatusCode >= 300) {
		fmt.Println("ERR: request to", fmt.Sprintf("%q", endpoint), "resulted in code", resp.StatusCode)
		os.Exit(1)
	}
